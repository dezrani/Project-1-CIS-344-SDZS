...
Purpose: Handles the booking form
...
<?php
    if ($_SERVER["REQUEST_METHOD"] == "POST") // If the request method is post, then set these variables
    {
        ...
        $firstname = htmlspecialchars($_POST["first-name"]); // htmlspecialchars converts to HTML entities so whatever is sent isn't seen as code.
        $lastname = htmlspecialchars($_POST["last-name"]);
        $email = htmlspecialchars($_POST["email"]);
        $artist = $_POST["artist"];
        $tattootype = $_POST["tattoo-type"];
        
        if (empty($firstname)) // If name is left empty, then we return to first page.
